...
function colorToHex(color) {
  ...
  if ((color.slice(0,1) == "#") && (color.length == 4)) {
    ...
    if ((!hexPair(a + a)) || (!hexPair(b + b)) || (!hexPair(c + c))) return null;  // validate input
    return "#" + a + a + b + b + c + c;
  ...
  // check for 6 digit hex - keep as is
    ...
    if ((!hexPair(a)) || (!hexPair(b)) || (!hexPair(c))) return null;  // validate input
    return color;
  ...
  // check for rgba or rgb value - drop alpha and convert to hex
    ...
    // validate input
    if ((d < 0) || (d > 255) || (e < 0) ||(e > 255) || (f < 0) || (f > 255)) return null;
  ...
  // maybe it's a named color
    ...
    for (let i = 0; i < colorNames.length; i++) {
      if (colorNames[i].toLowerCase() == color.toLowerCase()) return colorValues[i].toLowerCase();
    }
